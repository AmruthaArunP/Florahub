...
const securePassword = async (password) => {
    ...
    try {
        const passwordHash = await bcrypt.hash(password, 10);       //converting in to secure form
        return passwordHash;
    } catch (error) {
...
async function sendOtpMail(email, otp) {
    ...
    try {
        const transporter = nodemailer.createTransport({
            service: "gmail",
            auth: {
                user: "pamruthaarun@gmail.com",
                pass: "tajcdxqqsisgbiji",
            },
        });

        const mailOptions = {
            from: "pamruthaarun@gmail.com",
            to: email,
